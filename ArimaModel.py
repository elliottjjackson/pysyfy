...
class ArimaModel:
        ...
        for param in pdq:
            for param_seasonal in seasonal_pdq:
                try:
                    ...
                    mod = sm.tsa.statespace.SARIMAX(self.df, order=param,
                    seasonal_order=param_seasonal,
                    enforce_invertibility=False)
                    result = mod.fit(disp=0)
                    print(f'ARIMA{param}x{param_seasonal} - AIC: {result.aic}')
                    if self.param_dict['result.aic'] == None or result.aic < self.param_dict['result.aic']:
                    ...
                    continue
        model = sm.tsa.statespace.SARIMAX(self.df,
        order=self.param_dict['param'],
        seasonal_order=self.param_dict['param_seasonal'],
        enforce_invertibility=False)
        self.arima_model = model.fit()
        self.history_match = self.arima_model.get_prediction(start=pd.to_datetime(self.match_start_date),dynamic=False)
    ...
    def plot_history_match(self):
        ...
        self.history_match.predicted_mean.plot(ax=ax,label='One-step ahead Forecast', alpha=.7, figsize=(14,7))
